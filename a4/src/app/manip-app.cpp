...
bool I_AM_HAVING_SCREEN_ISSUES = true;
...
public:
	...
	virtual double evaluate(const VectorXd &u) const {
        ...
        if (QUESTION_12) {
            // Idea: penalize difference of orientation of connection between target points and the current handle bar
            // Can tune this regularization parameter
            double lambda = 0.5;

            auto x_prime = get_x_prime();
            auto I = sim->getFeaturePointsNodalIndices();
            auto delta_x_target = seg2(x_prime, I[1]) - seg2(x_prime, I[0]);
            double theta_target = std::atan2(delta_x_target[1],delta_x_target[0]);
            double theta_l = u[4];
            double theta_r = u[5];

            O += lambda*(std::pow(theta_target - theta_l, 2.0) + std::pow(theta_target - theta_r, 2.0));
        }
	...
	VectorXd get_DODu(const VectorXd &u) const {
		...
		VectorXd DODu; DODu.setZero(u.size()); {
			...
			if (step == FD_O_of_u) {
				...
				// TODO: MatrixXd dxdu = ...
				MatrixXd dxdu = MatrixXd::Zero(2,2);
                SparseMatrixd dFdx = -get_H(u,x);
				dxdu = solve_AX_EQUALS_B(dFdx, -dFdu);
				// --
				// TODO: DODu += ...
				...
				SparseMatrixd dFdu = get_dFdu(u, x); // TODO: Finish implementing get_dFdu(...)
				// TODO: MatrixXd dxdu = ...
                MatrixXd dxdu = MatrixXd::Zero(2,2);
                SparseMatrixd dFdx = -get_H(u,x);
                dxdu = solve_AX_EQUALS_B(dFdx, -dFdu);
	...
	VectorXd get_dOdx(const VectorXd &x) const {
		...
		// Only optimize over feature points
        for (const int &i : I) {
		...
		SparseMatrixd ret; ret.resize(x.size(), u.size()); {
			for (const auto &pin : sim->pins) { 
				...
				MatrixXd dp_primedu; dp_primedu.setZero(2, u.size()); {
                    ...
                    dp_primedu.block<2, 2>(0, xy_dataStartIndex) = MatrixXd::Identity(2,2);
					
					// => dp'dtheta = ???
					// TODO: dp_primedu.col(theta_dataIndex) = ...
					MatrixXd dRdtheta = MatrixXd::Zero(2,2);
					dRdtheta << -std::sin(theta), -std::cos(theta),
					            std::cos(theta), -std::sin(theta);
                    dp_primedu.col(theta_dataIndex) = dRdtheta * f;
				} 
