...
//        srand(1); // Used for plotting of Hamiltonian for different integration schemes
    ...
    int getClosest(int &boidIdx, bool groupToApproach) {
        T closestDistance = 10000;
        int closestBoidIdx = boidIdx;
        for (int i = 0; i < getParticleNumber(); i++) {
            if (i != boidIdx && groups(i) == groupToApproach && getDistance(boidIdx,i,positions) < closestDistance) {
                closestDistance = getDistance(boidIdx,i,positions);
                closestBoidIdx = i;
            }
        }
        assert(closestBoidIdx == boidIdx);
