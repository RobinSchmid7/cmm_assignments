...
bool USE_ZERO_INITIAL_GUESS = true; // If you would rather just use a zero initial guess (not a bad idea) :)
...
Vector2d get_Fk(const Vector2d &uk, const Vector2d &xk) {
	...
	// Gravitational force
	if (PLANET) {
	    Fk += (x_planet - xk).normalized() / (x_planet - xk).squaredNorm();
	}
...
public:
	...
	// Weight coefficients
    double log_xK_reg = 1.5; double log_xK_reg_min = -2.; double log_xK_reg_max = 2.; // Final position
    double log_vK_reg = 1.; double log_vK_reg_min = -2.; double log_vK_reg_max = 2.; // Final velocity
...
private: 
	...
	double ZOOM_ = .4f; // TODO: change this in the end to 1.f
	double ZOOM_MIN_ = .1f;
