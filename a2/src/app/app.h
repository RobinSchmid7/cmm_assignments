...
//        ground.draw(shadowShader, V3D(0.6,0.6,0.8));
        ...
        // For the looks
        drawRectangle(P3D(0,0,0), V3D(0,1,0), 0, Vector2d(100,100), shadowShader, V3D(0.8,0.7,0.1));
        drawSphere(P3D(0,-19,0), 20, shadowShader, V3D(0.8,0.7,0.2));
        std::random_device rd;
        std::mt19937 mt(rd());
        uniform_real_distribution<double> dist1(-50,50);
        uniform_real_distribution<double> dist2(1,3);
        uniform_real_distribution<double> dist3(0.5, 2.5);
        for (int i = 0; i < 150; i++) {
            double x = 0;
            double z = 0;
            int j = 0;
            ...
            // Trunk start from 1 to 3
            mt.seed(i);
            double y1 = dist2(mt);
            // Trunk end from 0.5 to 2.5
            mt.seed(2*i+1);
            double y2 = dist3(mt);
            double r = 0.2*y2;
            drawCylinder(P3D(x,0,z), P3D(x,y1+y2,z), r, shadowShader, V3D(0.1,0.8,0.2));
        }
    }
