...
protected:
    ...
    double currHeight = 0;
    double offsetHeight = -0.14; // Correction such that it looks smooth
    std::vector<double> footHeights;
...
public:
    ...
    void generateSteppingLocations() {
        ...
        bFrameMotionPlan.populateFootstepPlan(fsp, lmProps, &cpm, footHeights);
    }

    double getGroundHeight(double x, double z, double rBall = 20, double maxHeight = 1) {
//        return 0; // For flat ground simulation, i.e. task 1 -3
        currHeight = sqrt(rBall*rBall - x*x - z*z) + maxHeight - rBall + offsetHeight;
        if (currHeight < 0) return 0; // Prevent sinking in ground
        return currHeight;
    }

    void setFootHeights() {
        // Order with limbs get called: fl, hl, fr, hr - populate footsteps this way
        for (uint i = 0; i < robot->limbs.size(); i++) {
            footHeights[i] = getGroundHeight(robot->limbs[i]->eeRB->state.pos.x, robot->limbs[i]->eeRB->state.pos.z);
        }
    }
    ...
    virtual void generateTrajectoriesFromCurrentState(double dt = 1 / 30.0) {
        groundHeight = getGroundHeight(robot->root->state.pos.x, robot->root->state.pos.z, 20, 1);
