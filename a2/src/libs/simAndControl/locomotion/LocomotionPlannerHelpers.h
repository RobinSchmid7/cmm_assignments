...
public:
    ...
    Trajectory3D generateLimbTrajectory(RobotLimb* limb,
        ...
        // Update footheights
        double fh = 0;
        if ((string)(limb->name) == "fl") fh = footHeights[0];
        if ((string)(limb->name) == "hl") fh = footHeights[1];
        if ((string)(limb->name) == "fr") fh = footHeights[2];
        if ((string)(limb->name) == "hr") fh = footHeights[3];
...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            pos = pos + heading*((robot->forward)*vForward + RBGlobals::worldUp.cross(robot->forward)*vSideways)*dt; /* + TODO: fix this */;
